...
for(i in 1:length(file_names)) {
  ...
  file.rename(from = here::here(output_folder, "history.csv"), 
...
if(!(file.exists(here::here(here(), "rawdata/gps_mail/eez_serpam/log_process.txt")))){
  file.create(here::here(here(), "rawdata/gps_mail/eez_serpam/log_process.txt"))
}
...
texto <- glue::glue(
  '## Log date {momentum}
  ## Files processed: {length(file_names)}
  ### IDs of the files processed: 
  {glue::glue_collapse(basename(file_names), sep = ", ")}
  
  ## Date period: {month_date}
  ### First record date: {min(data$time_stamp)} 
  ### Last record date: {max(data$time_stamp)}
  
  ## GPS devices processed: {length(unique(data$id_collar))} 
  ## Users devices: {length(unique(data$id_user))} users
  ## {nrow(data)} records were processed
  
  ### GPS devices:
  {glue::glue_collapse(unique(data$id_collar), sep = ", ")}
  
  ### Output data: {paste0("rawdata/gps_mail/all_gps_", month_date, ".csv")} 
  =============================================================')

write(texto, here::here(here(), "rawdata/gps_mail/eez_serpam/log_process.txt"), 
      append = TRUE)
